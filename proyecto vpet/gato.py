...
ruta_imagenes = r'F:\2 curso\pagina web\Proyecto_python\proyecto vpet\sprites_gato\\'
...
def evento(ciclo, estado, numero_evento, pos_x):
    if numero_evento in numeros_idle:
        estado = 0
        ventana.after(400, actualizar, ciclo, estado, numero_evento, pos_x)
...
def actualizar(ciclo, estado, numero_evento, pos_x):
    ...
    ventana.geometry(f'100x100+{pos_x}+{pos_y}')
...
#los gifs de los estados de la mascota 
idle = [tk.PhotoImage(file=ruta_imagenes + 'idle.gif', format='gif -index %i' % i) for i in range(5)]
idle_a_dormir = [tk.PhotoImage(file=ruta_imagenes + 'idle_to_sleep.gif', format='gif -index %i' % i) for i in range(8)]
dormir = [tk.PhotoImage(file=ruta_imagenes + 'sleep.gif', format='gif -index %i' % i) for i in range(3)]
dormir_a_idle = [tk.PhotoImage(file=ruta_imagenes + 'sleep_to_idle.gif', format='gif -index %i' % i) for i in range(8)]
caminar_izq = [tk.PhotoImage(file=ruta_imagenes + 'walking_positive.gif', format='gif -index %i' % i) for i in range(8)]
caminar_der = [tk.PhotoImage(file=ruta_imagenes + 'walking_negative.gif', format='gif -index %i' % i) for i in range(8)]
